...
function findAllIndex(str,queries){
    return queries.reduce((indexes,el,i)=>{
     if(el===str){
     indexes.push(i);
    }
    return indexes;
...
for (let i = 0; i < queries.length; i++) {
    ...
    const foundIndexes= findAllIndex(str,queries);
    foundIndexes.map(index =>{
        freqArr[index]++;
    })
});
return freqArr;
